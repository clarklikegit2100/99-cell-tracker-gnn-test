    ...
    try:
        for T, image in enumerate(dataset):
            ...
            if not params.dry_run:
                                ...
                                try:
                                    ...
                                    for c in unique_centers:
                                        ...
                                        if order > 0:
                                            ...
                                            d1 = skfmm.distance(phi) * (1 - this_c)
                                            d0 = 0
                ...
                log_print("Saved File: {}".format(out_fname))
                # for l in np.unique(labels_out):
                #     if l == 0:
                #         continue
                #     statst = cv2.connectedComponentsWithStats((labels_out == l).astype(np.uint8), connectivity=8,
                #                                               ltype=cv2.CV_32S)
                #     if statst[0] > 2:
                #         print(l, statst[0], statst[2][:, cv2.CC_STAT_AREA], (labels_out == l).sum())
                if params.save_intermediate:
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument('--data_format', dest='data_format', type=str, choices=['NCHW', 'NHWC'],
