        ...
        for T, image in enumerate(dataset):
            ...
            if not params.dry_run:
                ...
                for n_id, n in enumerate(np.unique(labels)):
                    ...
                    #                                                  seg_edge_crop))
                    bw_crop2 = bw_crop
                    for iter in range(params.edge_dist):

                        # dilate 30x with it
                        bw_dilate = ndi.binary_dilation(bw_crop2, diamond)
                        bw_crop2 = np.logical_or(bw_crop,  np.logical_and(bw_dilate,seg_edge_crop))
                    ...
                    bw_fill = bbox_fill(bw, bw_crop, loc, three_d=True)
                    labels[bw_fill] = n
                    # _, n_n_ = scipy.ndimage.label((labels==n).astype(np.uint8))
                    # if n_n_ > 2:
                    #     print(f'n: {n_n_}')
                        ...
                        if len(unique_centers) > 1:
                            ...
                            if len(d_c) > 1:
                                ...
                                for nc in range(len(d_c)):
                                    _, num_new_l_c = scipy.ndimage.label((new_label == nc + 1).astype(np.uint8))
                                    if num_new_l_c > 2:
                                        skip = True
                                        break
                                if skip:
                                ...
                                for nl in np.unique(new_label):
                                    ...
                                    # _, n_n_ = scipy.ndimage.label((labels == new_global_l).astype(np.uint8))
                                    # if n_n_ > 2:
                                    #     print(f'l: {new_global_l}, n: {n_n_}')
