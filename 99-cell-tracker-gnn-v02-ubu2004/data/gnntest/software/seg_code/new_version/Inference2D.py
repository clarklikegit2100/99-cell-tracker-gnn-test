        ...
        for T, image in enumerate(dataset):
            ...
            if not params.dry_run:
                ...
                for l in np.unique(labels):
                    if l == 0:
                        continue
                    statst = cv2.connectedComponentsWithStats((labels == l).astype(np.uint8), connectivity=8,
                                                              ltype=cv2.CV_32S)
                    if statst[0] > 2:
                        labels[labels == l] = labels_[labels == l]

                # same process from center cells, assuming they are all seperated
                if add_binary_output:
                    centers_hard = np.greater(center_sigmoid_np, params.centers_sigmoid_threshold)
                    # TODO add fine filter
                ...
                for n in range(1, num_cells):
                    ...
                    fill_crop = scipy.ndimage.morphology.binary_fill_holes(bw_crop).astype(np.float32)
                    bw_full = bbox_fill(bw, fill_crop, loc)
                    labels[bw_full] = n
                    ...
                    if add_binary_output and not params.no_tra:
                        ...
                        # if there is more then 1 center in the croped image of the full labeled cell then ceparate them usnig fast marching
                        if len(unique_centers) > 1:
                            ...
                            if len(d_c) > 1:
                                d_c = np.stack(d_c, 0)
                                d_c = np.maximum(d_c, 0)
                                new_label = np.argmin(d_c, axis=0) + 1
                                new_label = new_label*fill_crop
                                skip = False
                                for nc in range(len(d_c)):
                                    new_label_cc = cv2.connectedComponentsWithStats((new_label == nc + 1).astype(np.uint8), 8,
                                                                                    cv2.CV_32S)
                                    if new_label_cc[0] > 2:
                                        skip = True
                                        break
                            ...
                            # if params.must_have_cnt:
                            #     # if no big enough center discard cell
                            #     if len(d_c) == 0:
                            #         labels[labels == n] = 0
                        if params.must_have_cnt:
                ...
                if params.FOV:
                    ...
                    if params.FOV == 25:   # Hela
                        remove_ind = np.setdiff1d(np.unique(labels), unique_fov_labels) # remove labels that are not in the FOV
                    else:
                ...
                if params.save_intermediate:
                    ...
                    R[centers_hard == 1] = 255
                    B[centers_hard == 1] = 0
                    G[centers_hard == 1] = 0
                    merge = np.stack([B, G, R], axis=2).astype(np.uint8)
                    ##
