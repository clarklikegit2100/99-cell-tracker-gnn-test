...
class CTCRAMReaderSequence2D(object):
    ...
    def _read_sequence_to_ram_(self):
        for sequence_folder in self.sequence_folder_list:
            ...
            original_size = 0
            downampled_size = 0
            remove_seg_list = np.random.rand(len(filename_list)) > keep_rate
            for t, (filename, remove_seg) in enumerate(zip(filename_list, remove_seg_list)):
                remove_tra = remove_seg if self.remove_tra else False
                ...
                if self.output_tra:
                    ...
                    tra = np.minimum(tra, 1)  # get BW image
                    # in case of no TRA input create tra using seg
                    # tra = np.zeros_like(seg)
                    #
                    # if self.erode_dilate_tra[1] < 0:
                    #     kernel = np.zeros((-self.erode_dilate_tra[1], -self.erode_dilate_tra[1]), dtype=tra.dtype)
                    #     erode = grey_erosion(seg, structure=kernel)
                    #     dilate = grey_dilation(seg, structure=kernel)
                    #     tra = erode.copy()
                    #     tra[np.not_equal(erode, dilate)] = 0
                    # elif self.erode_dilate_tra[1] > 0:
                    #     kernel = np.zeros((self.erode_dilate_tra[1], self.erode_dilate_tra[1]), dtype=tra.dtype)
                    #     tra = grey_dilation(seg, structure=kernel)
                    # tra[seg == -1] = -1
                    ...
                    #             continue
                    #         if self.erode_dilate_tra[1] < 0:
                    #             kernel = np.zeros((-self.erode_dilate_tra[1], -self.erode_dilate_tra[1]),
                    #                               dtype=tra.dtype)
                    #
                    #             tra += grey_erosion(seg == c, structure=kernel)*c
                    #         elif self.erode_dilate_tra[1] > 0:
                    #             kernel = np.zeros((self.erode_dilate_tra[1], self.erode_dilate_tra[1]),
                    #                               dtype=tra.dtype)
                    #             tra += grey_dilation(seg == c, structure=kernel)*c
                    # else:
                    #     if filename[2] is None or not keep_seg:
                    #         tra = np.ones(img.shape[:2]) * (-1)
                    #     else:
                    #         tra = cv2.imread(os.path.join(sequence_folder, filename[2]), -1)
                    #         if self.erode_dilate_tra[0] < 0:
                    #             kernel = np.zeros((-self.erode_dilate_tra[0], -self.erode_dilate_tra[0]), dtype=tra.dtype)
                    #             tra = grey_erosion(tra, structure=kernel)
                    #         elif self.erode_dilate_tra[0] > 0:
                    #             kernel = np.zeros((self.erode_dilate_tra[0], self.erode_dilate_tra[0]), dtype=tra.dtype)
                    #             tra = grey_dilation(tra, structure=kernel)

                    all_tra[t] = tra.astype(np.float32) if not remove_tra else -np.ones_like(seg)
                all_images[t] = img
                all_seg[t] = seg if not remove_seg else -np.ones_like(seg)
    ...
    def _load_and_enqueue(self, q, q_stat):
        ...
        try:
            while not self.coord.should_stop():
                ...
                # use np.random.normal(mu, sigma) with mu =256, sigma =80 and abs for - values
                ...
                if self.elastic_augmentation:
                    ...
                    # # can set boundary using the crop size as follows:
                    # bb_x = int(self.sub_seq_size[0]/0.84)
                    # bb_y = int(self.sub_seq_size[1]/0.84)
                    # boundary_x = int(bb_x * 0.08)
                    # boundary_y = int(bb_y * 0.08)
                    affine_size = (self.sub_seq_size[0]+2*boundary_y, self.sub_seq_size[1]+2*boundary_x)

                    # # get min size for affine transform before croping for augmentation
                    # if self.sub_seq_size[0]*2 >= img_size[0] or self.sub_seq_size[1]*2 >= img_size[1]:
                    #     # this is to get a simetric affine size
                    #     new_size = min(img_size[0], img_size[1])
                    #     affine_size = (new_size, new_size)
                    # else:
                    #     affine_size = (self.sub_seq_size[0]*2, self.sub_seq_size[1]*2)
                ...
                for t, file_idx in enumerate(filename_idx):
                    ...
                    # if we didnt random egde for crop we use affine transformation
                    # if not (crop_x < boundary_x or crop_x_stop > img_size[1] - boundary_x or
                    #         crop_y < boundary_y or crop_y_stop > img_size[0] - boundary_y):
                    if not boundary_flag:
                        if self.elastic_augmentation:
                            ...
                            # seg_crop = seg_crop[crop_y:crop_y_stop, crop_x:crop_x_stop]
                            ...
                            if self.output_tra:
                                ...
                                # tra_crop = tra_crop[crop_y:crop_y_stop, crop_x:crop_x_stop]
                        else:
