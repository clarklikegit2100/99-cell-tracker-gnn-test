...
#
ROOT_DATA_DIR = '/media/rrtammyfs/labDatabase/CellTrackingChallenge/Training/Silver_GT'
# ROOT_DATA_DIR = '/media/rrtammyfs/labDatabase/CellTrackingChallenge/Training'
ROOT_SAVE_DIR = '/media/rrtammyfs/Users/shaked0/LSTMUnet'
...
class ParamsBase(object):
    ...
    add_tra_output = True # assafs model uses this argument for inference and training:
    send_email = True
    email_username = 'bgumedicalimaginglab@gmail.com'
    email_password = "xmvipxhcpxvbeamc"
    receiver_email = 'shaked0@post.bgu.ac.il'
    keep_seg = 1
    remove_tra = False
...
class CTCParams(ParamsBase):
    # --------General-------------
    dataset_name = 'PhC-C2DH-U373-ST'
    experiment_name = 'TraFixU373'
    ...
    # train_sequence_list = [('Fluo-N2DH-SIM+', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('Fluo-N2DH-SIM+', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('Fluo-C2DL-MSC', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('Fluo-C2DL-MSC', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('DIC-C2DH-HeLa', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('DIC-C2DH-HeLa', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('Fluo-N2DH-GOWT1', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('Fluo-N2DH-GOWT1', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('Fluo-N2DL-HeLa', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('Fluo-N2DL-HeLa', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('PhC-C2DH-U373', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('PhC-C2DH-U373', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('PhC-C2DL-PSC', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('PhC-C2DL-PSC', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('DIC-C2DH-HeLa-ST', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('DIC-C2DH-HeLa-ST', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('PhC-C2DH-U373-ST', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('PhC-C2DH-U373-ST', '{:02d}'.format(s)) for s in range(1, 3)]
    # train_sequence_list = [('PhC-C2DL-PSC-ST', '{:02d}'.format(s)) for s in range(1, 3)]
    # val_sequence_list = [('PhC-C2DL-PSC-ST', '{:02d}'.format(s)) for s in range(1, 3)]
    ...
    # using new train validation split only for ST datasets:
    # train_sequence_list = [('PhC-C2DL-PSC-ST', '{:02d}'.format(s)) for s in [10, 12, 20, 22]]
    # val_sequence_list = [('PhC-C2DL-PSC-ST', '{:02d}'.format(s)) for s in [11, 21]]
    train_sequence_list = [('PhC-C2DH-U373-ST', '{:02d}'.format(s)) for s in [10,12,20,22]]
    val_sequence_list = [('PhC-C2DH-U373-ST', '{:02d}'.format(s)) for s in [11,21]]
    # train_sequence_list = [('Fluo-N2DL-HeLa-ST', '{:02d}'.format(s)) for s in [10, 12, 20, 22]]
    # val_sequence_list = [('Fluo-N2DL-HeLa-ST', '{:02d}'.format(s)) for s in [11, 21]]
    # train_sequence_list = [('DIC-C2DH-HeLa-ST', '{:02d}'.format(s)) for s in [10, 12, 20, 22]]
    # val_sequence_list = [('DIC-C2DH-HeLa-ST', '{:02d}'.format(s)) for s in [11, 21]]
    # train_sequence_list = [('BF-C2DL-HSC-ST', '{:02d}'.format(s)) for s in [10, 12, 20, 22]]
    # val_sequence_list = [('BF-C2DL-HSC-ST', '{:02d}'.format(s)) for s in [11, 21]]
    # train_sequence_list = [('BF-C2DL-MuSC-ST', '{:02d}'.format(s)) for s in [10, 12, 20, 22]]
    # val_sequence_list = [('BF-C2DL-MuSC-ST', '{:02d}'.format(s)) for s in [11, 21]]
    # train_sequence_list = [('Fluo-N2DH-GOWT1-ST', '{:02d}'.format(s)) for s in [10, 12, 20, 22]]
    # val_sequence_list = [('Fluo-N2DH-GOWT1-ST', '{:02d}'.format(s)) for s in [11, 21]]
    # train_sequence_list = [('Fluo-C2DL-MSC-ST', '{:02d}'.format(s)) for s in [10, 12, 20, 22]]
    # val_sequence_list = [('Fluo-C2DL-MSC-ST', '{:02d}'.format(s)) for s in [11, 21]]

    # from asaf need to check
    # train_sequence_list = [('Fluo-N2DH-MYSIM', '{:02d}'.format(s)) for s in range(1,45)]
    # val_sequence_list = [('Fluo-N2DH-MYSIM', '{:02d}'.format(s)) for s in range(45, 52)]
    # add_tra_output = False
    add_tra_output = True # assafs model uses this argument for inference and training:
    erode_dilate_tra = (-8, -8)
    crop_size = (256, 256)
    batch_size = 5
    unroll_len = 4
    data_format = 'NCHW'
    ...
    learning_rate = 1e-5
    num_iterations = 200000 # for regualar run
    ...
    # load_checkpoint_path = ROOT_SAVE_DIR
...
class CTCInferenceParams(ParamsBase):
    ...
    centers_sigmoid_threshold = 0.2 # for binrization of TRA output (soft to BW image)
    min_center_size = 1 # for center blob analysis, maybe add max as well
...
class CTCInferenceParamsSearch(ParamsBase):

    gpu_id = 1  # for CPU ise -1 otherwise gpu id
    seq = 1
    no_tra = False
    model_path = '/media/rrtammyfs/Users/shaked0/LSTMUnet/LSTMUNet2D/Fluo-C2DL-Huh7/TraFixHuh7/2021-07-16_112400'
    # output_path = '/media/rrtammyfs/labDatabase/CellTrackingChallenge/Training/Fluo-C2DL-Huh7/{:02d}_RES'.format(seq)
    sequence_path = '/media/rrtammyfs/labDatabase/CellTrackingChallenge/Training/Fluo-C2DL-Huh7/{:02d}'.format(seq)

    filename_format = 't*.tif'  # default format for CTC
    gt_format = '{}_GT/SEG/man_seg{:03d}.tif'  # default format for CTC
    digit_4 = False  # for 4 digit format datasets
    edge_thresholds = np.arange(0.1, 0.5, 0.1)
    data_reader_time = DataHandeling.CTCInferenceReaderTime  # for test time augmentations
    data_reader = DataHandeling.CTCInferenceReader
    # field of view prameters from website
    FOV = 50
    ...
    edge_dists = np.arange(0, 20, 1)
    pre_sequence_frames = 4  # LSTM history for first frames, done using mirror padding (in time) of this size
    centers_sigmoid_thresholds = np.arange(0.05, 0.5, 0.05) # for binrization of TRA output (soft to BW image)
    min_center_sizes = np.arange(5, 55, 5) # for center blob analysis, maybe add max as well
