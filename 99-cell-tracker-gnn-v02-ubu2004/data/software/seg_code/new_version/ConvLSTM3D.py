...
try:
    ...
    from tensorflow.keras.layers.recurrent import _standardize_args
...
class ConvRNN3D(RNN):
  ...
  Input shape:
    ...
    `(samples, timesteps, channels, depth, rows, cols)`
    if data_format='channels_first' or 6D tensor with shape:
    `(samples, timesteps, depth, rows, cols, channels)`
  ...
  Output shape:
    - If `return_state`: a list of tensors. The first tensor is
      ...
      `(samples, filters, new_depth, new_rows, new_cols)`
      if data_format='channels_first'
      or 5D tensor with shape:
      `(samples, new_depth, new_rows, new_cols, filters)`
      if data_format='channels_last'.
      `rows` and `cols` values might have changed due to padding.
    - If `return_sequences`: 6D tensor with shape:
      `(samples, timesteps, filters, new_depth, new_rows, new_cols)`
      if data_format='channels_first'
      or 6D tensor with shape:
      `(samples, timesteps, new_depth, new_rows, new_cols, filters)`
      if data_format='channels_last'.
    - Else, 5D tensor with shape:
      `(samples, filters, new_depth, new_rows, new_cols)`
      if data_format='channels_first'
      or 5D tensor with shape:
      `(samples, new_depth, new_rows, new_cols, filters)`
      ...
      - Specify a fixed batch size for your model, by passing
         ...
         - If functional model with 1 or more Input layers:
            ...
            *including the batch size*.
            ...
            e.g. `(32, 10, 100, 100, 100, 32)`.
            Note that the number of depth, rows and columns should be specified
            too.
  ...
  def compute_output_shape(self, input_shape):
    ...
    if cell.data_format == 'channels_first':
      output_shape = input_shape[:2] + (cell.filters, depth, rows, cols)
    elif cell.data_format == 'channels_last':
      output_shape = input_shape[:2] + (depth, rows, cols, cell.filters)
    ...
    if self.return_state:
      ...
      if cell.data_format == 'channels_first':
        output_shape += [(input_shape[0], cell.filters, depth, rows, cols)
                         for _ in range(2)]
      elif cell.data_format == 'channels_last':
        output_shape += [(input_shape[0], depth, rows, cols, cell.filters)
                         for _ in range(2)]
    return output_shape
...
class ConvLSTM3DCell(DropoutRNNCellMixin, Layer):
  ...
  def call(self, inputs, states, training=None):
    ...
    (kernel_i, kernel_f,
     kernel_c, kernel_o) = array_ops.split(self.kernel, 4, axis=4)
    (recurrent_kernel_i,
     recurrent_kernel_f,
     recurrent_kernel_c,
     recurrent_kernel_o) = array_ops.split(self.recurrent_kernel, 4, axis=4)
  ...
  def input_conv(self, x, w, b=None, padding='valid'):
    conv_out = K.conv3d(x, w, strides=self.strides,
                        padding=padding,
  ...
  def recurrent_conv(self, x, w):
    conv_out = K.conv3d(x, w, strides=(1, 1, 1),
                        padding='same',
                        data_format=self.data_format)
    return conv_out
  ...
  def get_config(self):
    ...
    base_config = super(ConvLSTM3DCell, self).get_config()
    return dict(list(base_config.items()) + list(config.items()))


@keras_export('keras.layers.ConvLSTM3D')
class ConvLSTM3D(ConvRNN3D):
  """Convolutional LSTM.

  It is similar to an LSTM layer, but the input transformations
  and recurrent transformations are both 3D convolutional.
  ...
  Input shape:
    - If data_format='channels_first'
        6D tensor with shape:
        `(samples, time, channels, depth, rows, cols)`
    - If data_format='channels_last'
        6D tensor with shape:
        `(samples, time, depth, rows, cols, channels)`

  Output shape:
    - If `return_sequences`
       - If data_format='channels_first'
          6D tensor with shape:
          `(samples, time, filters, output_depth, output_row, output_col)`
       - If data_format='channels_last'
          6D tensor with shape:
          `(samples, time, output_depth, output_row, output_col, filters)`
    - Else
      - If data_format ='channels_first'
          5D tensor with shape:
          `(samples, filters, output_depth, output_row, output_col)`
      - If data_format='channels_last'
          5D tensor with shape:
          `(samples, output_depth, output_row, output_col, filters)`
      where 'o_depth', `o_row` and `o_col` depend on the shape of the filter and
      the padding
  ...
  def __init__(self,
               ...
               recurrent_dropout=0.,
               **kwargs):
    cell = ConvLSTM3DCell(filters=filters,
    ...
    super(ConvLSTM3D, self).__init__(cell,
  ...
  def call(self, inputs, mask=None, training=None, initial_state=None):
    ...
    return super(ConvLSTM3D, self).call(inputs,
                                        mask=mask,
                                        training=training,
  ...
  def get_config(self):
    config = {'filters': self.filters,
              ...
              'dropout': self.dropout,
              'recurrent_dropout': self.recurrent_dropout}
    base_config = super(ConvLSTM3D, self).get_config()
