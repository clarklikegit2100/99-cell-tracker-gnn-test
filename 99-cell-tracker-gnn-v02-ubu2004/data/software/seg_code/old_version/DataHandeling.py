...
class CTCRAMReaderSequence2D(object):
    def __init__(self, sequence_folder_list, image_crop_size=(128, 128), unroll_len=7, deal_with_end=0, batch_size=4,
                 ...
                 output_tra=False, erode_dilate_tra=0):
                ...
                if self.output_tra:
                    if filename[2] is None or not keep_seg:
                        ...
                        if self.erode_dilate_tra < 0:
                            kernel = np.zeros((-self.erode_dilate_tra, -self.erode_dilate_tra), dtype=tra.dtype)
                            tra = grey_erosion(tra, structure=kernel)
                        elif self.erode_dilate_tra > 0:
                            kernel = np.zeros((self.erode_dilate_tra, self.erode_dilate_tra), dtype=tra.dtype)
                            tra = grey_dilation(tra, structure=kernel)

                    all_tra[t] = tra.astype(np.float32)
                all_images[t] = img
                all_seg[t] = seg
        ...
        def gen():
            ...
            for file in full_file_list:
                ...
                if normalize:
                    ...
                    img = img / (img.std())
                if depth_pad:
                    img = np.pad(img, ((depth_pad, depth_pad), (0, 0), (0, 0)))
                yield img
...
class CTCRAMReaderSequence3DSlice(object):
    def __init__(self, sequence_folder_list, image_crop_size=(128, 128), unroll_len=7, deal_with_end=0, batch_size=4,
                 queue_capacity=32, num_threads=3,
                 data_format='NCHW', randomize=True, return_dist=False, keep_sample=1, elastic_augmentation=True,
                 depth=0, load_to_ram=True, output_tra=False, erode_dilate_tra=0):
        if not isinstance(image_crop_size, tuple):
            image_crop_size = tuple(image_crop_size)
        self.coord = None
        self.unroll_len = unroll_len
        ...
        if load_to_ram:
            ...
            if self.output_tra:
                if tra_data_path is None:
                    ...
                    tra = utils.read_multi_tiff(tra_data_path, start_z=start_z, stop_z=stop_z).astype(np.float32)
                if tra_data_path is not None and not np.any(tra):
