...
MODEL_PYTORCH_LIGHTNING = (
    posix(f"{log_abs}/runs/2025-10-09/13-00-43/checkpoints/epoch=43.ckpt")
)
...
run_cmd([
    ...
    "-cs", "20",
    "-ii", posix(f"{dataset_abs}/{SEQUENCE}"),
    "-iseg", posix(f"{dataset_abs}/{SEQUENCE}_GT/TRA"),
    "-im", MODEL_METRIC_LEARNING,
    "-oc", posix(f"{dataset_abs}/{SEQUENCE}_CSV"),
])
...
run_cmd([
    ...
    "-modality", MODALITY,
    "-iseg", posix(f"{dataset_abs}/{SEQUENCE}_GT/TRA"),
    "-oi", posix(f"{dataset_abs}/{SEQUENCE}_RES_inference"),
])
