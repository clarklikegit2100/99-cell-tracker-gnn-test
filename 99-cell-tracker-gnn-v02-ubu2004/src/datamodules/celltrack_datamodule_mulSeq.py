...
class CellTrackDataModule(LightningDataModule):
    ...
    # Assuming each of self.data_train, self.data_val, self.data_test is a list of dicts or namedtuples
    def save_datasets_to_csv(self):
        def extract_data(subset, name):
            records = []
            for idx in subset.indices:
                data_item = self.data[idx]
                # Assumes data_item is a dict or has a .__dict__ or .x/y/edge_index, etc.
                if isinstance(data_item, dict):
                    records.append(data_item)
                elif hasattr(data_item, '__dict__'):
                    records.append(vars(data_item))
                else:
                    # Customize this based on actual structure of your data_item
                    records.append({
                        "x": data_item.x.tolist() if hasattr(data_item, "x") else None,
                        "y": data_item.y.tolist() if hasattr(data_item, "y") else None,
                        "edge_index": data_item.edge_index.tolist() if hasattr(data_item, "edge_index") else None,
                    })
            df = pd.DataFrame(records)
            df.to_csv(f"{name}.csv", index=False)
            print(f"{name}.csv saved with {len(records)} rows")

        extract_data(self.data_train, "data_train")
        extract_data(self.data_val, "data_val")
        extract_data(self.data_test, "data_test")
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        # Call it somewhere in your training script
        print('save begin')
        self.save_datasets_to_csv()
        print('save end')
